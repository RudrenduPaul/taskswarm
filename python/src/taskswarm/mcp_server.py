...
_FALLBACK_DESCRIPTION = (
    "Runs the taskswarm CLI with the given argument list and returns its parsed JSON "
    "output. taskswarm is a self-hosted, event-driven coordination server for parallel "
    "coding-agent sessions (Claude Code, Codex, Cursor): it fires a local OS notification "
    "and updates a live status page the instant a session blocks, needs review, fails, or "
    "finishes. Commands: 'start', 'task add --title <t> --repo <path>', 'task list', "
    "'agent report-status --task <id> --repo <path> --state <state>', 'token rotate', and "
    "'hooks install <adapter>'. Most accept '--json'. Always pass '--json' when the "
    "subcommand supports it so the output can be parsed as structured data, e.g. "
    "run(args=['task', 'list', '--json'])."
)


def _build_tool_description() -> str:
    """Builds the `run` tool's description from the real `taskswarm --help` output at
    ...
    return (
        "Runs the taskswarm CLI with the given argument list and returns its parsed "
        f"JSON output. Always pass '--json' when the subcommand supports it. Real "
        f"`taskswarm --help` output:\n\n{help_text}"
    )
